/**
 * Adapted from: https://github.com/steambap/png-to-ico
 */
import * as Jimp from 'jimp';
import {JimpImage, Image} from '../types';

export async function createIco(images: Image[]) {
	...
	for (const image of images) {
		const jimp = await Jimp.read(image.buffer);
		const dir = getDir(jimp, offset);
		const bmpInfoHeader = getBmpInfoHeader(jimp);
		const dib = getDib(jimp);
...
function getDir(img: JimpImage, offset: number) {
...
function getBmpInfoHeader(img: JimpImage) {
